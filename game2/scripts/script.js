...
const nextLevel = gameModal.querySelector("a.invisible");
const nextLevelHint = gameModal.querySelector("p.invisible");
...
let wordListArr = new Array(...wordList);
...
const getRandomWord = () => {
  ...
  const randomWordIndex = Math.floor(Math.random() * wordListArr.length);
  // Selecting a random word and hint from the wordList
  const { word, hint } = wordListArr[randomWordIndex];
  wordListArr.splice(randomWordIndex, 1);
...
const gameOver = (isVictory) => {
  ...
  if (points === 3) {
    ...
    nextLevelHint?.classList.remove("invisible");
  }
